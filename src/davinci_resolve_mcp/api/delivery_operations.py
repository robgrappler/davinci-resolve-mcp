...
def add_to_render_queue(
    ...
    preset_valid, available_presets, preset_message = validate_render_preset(render_settings_interface, preset_name)
    if not preset_valid:
...
def start_render(resolve) -> Dict[str, Any]:
    ...
    try:
        ...
        if result:
            ...
            return {"success": True, "message": "Render started successfully", "jobs_count": len(queue_items)}
        else:
...
def get_render_queue_status(resolve) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": queue_status, "jobs_count": len(jobs), "jobs": jobs, "is_rendering": is_rendering}
...
def clear_render_queue(resolve) -> Dict[str, Any]:
    ...
    try:
        ...
        if initial_count == 0:
            return {"success": True, "message": "Render queue is already empty", "jobs_removed": 0}
