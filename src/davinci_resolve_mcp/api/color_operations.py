...
def add_node(resolve, node_type: str = "serial", label: str = None) -> str:
    ...
    try:
        ...
        if not current_grade:
            ...
            return (
                f"Error adding {node_type} node: Cannot access grade object. The clip may not be properly graded yet."
            )
...
def set_color_wheel_param(resolve, wheel: str, param: str, value: float, node_index: int = None) -> str:
    ...
    wheel_to_function_prefix = {"lift": "SetLift", "gamma": "SetGamma", "gain": "SetGain", "offset": "SetOffset"}
