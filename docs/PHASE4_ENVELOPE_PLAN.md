...
6. python -m pytest tests/unit/ -q  → must pass
   ruff check src/ tests/unit/ && ruff format --check src/ tests/unit/  → must pass

7. git commit -m "phase 4 batch N: migrate <domain> tools to response envelope"
   git push -u origin claude/phase-4-envelope-migration

8. Repeat from step 2 until all batches are checked.

9. When all batches complete: open a PR against main titled
   "Phase 4: uniform response envelope + v2.0.0 bump"
```
...
| File | Purpose |
|---|---|
| `src/davinci_resolve_mcp/utils/response.py` | The envelope helpers — source of truth |
| `src/davinci_resolve_mcp/handlers/delivery.py` | Reference: partial migration already done |
| `src/davinci_resolve_mcp/handlers/system.py` | Start here (Batch 1 — simplest) |
| `docs/PHASE4_ENVELOPE_PLAN.md` | This file — tick off batches as you go |
| `tests/unit/` | Run after every commit |

### What NOT to change

- `@resource()` functions — leave return types as-is (plain Python objects)
- `src/davinci_resolve_mcp/api/` functions — internal layer, not exposed to MCP clients
- `src/davinci_resolve_mcp/utils/response.py` — only add `warn_response` in Batch 0, no other changes
- Test files other than adding new ones
